    ...
    <div class="bg-white p-8 rounded shadow-md w-full max-w-md">
        ...
        <form action="{{ route('admin.produks.update', ['id' => $produk->id_produk]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama_Produk" id="nama_Produk" value="{{ $produk->nama_Produk }}" class="w-full border rounded px-3 py-2">
            </div>
            
            <div class="mb-4">
                <label for="deskripsi" class="block text-gray-600">Deskripsi</label>
                <input type="text" name="deskripsi" id="deskripsi" value="{{ $produk->deskripsi }}" class="w-full border rounded px-3 py-2">
            </div>
            
            <div class="mb-4">
                <label for="stok" class="block text-gray-600">Stok</label>
                <input type="number" name="stok" id="stok" value="{{ $produk->stok }}" class="w-full border rounded px-3 py-2">
            </div>
            
            <div class="mb-4">
                <label for="harga" class="block text-gray-600">Harga</label>
                <input type="number" name="harga" id="harga" value="{{ $produk->harga }}" class="w-full border rounded px-3 py-2">
            </div>
